...
impl ContractInterface for Contract {
    fn validate_state(
        ...
        if bytes.is_empty() {
            info!("[VALIDATE_STATE] Empty state, returning Valid");
        ...
        info!("[VALIDATE_STATE] State validated successfully: {:?}", ping);

        StarkVerifier::verify(&[], &[], &[]).unwrap();
    ...
    fn update_state(
        ...
        info!("[UPDATE_STATE] Returning final state: {:?}", ping);
    ...
    fn summarize_state(
        ...
        info!("[SUMMARIZE_STATE] State size: {}", state.as_ref().len());

        let state = state.as_ref();
        if state.is_empty() {
            info!("[SUMMARIZE_STATE] Empty state, returning empty summary");
        ...
        info!("[SUMMARIZE_STATE] State summarized: {:?}", ping);
    ...
    fn get_state_delta(
        ...
        info!("[GET_STATE_DELTA] Contract options: {:?}", opts);

        let mut ping = if state.is_empty() {
            info!("[GET_STATE_DELTA] Empty state, using default Ping");
            Ping::default()
        } else {
            let p = serde_json::from_slice::<Ping>(state.as_ref())
                .map_err(|e| ContractError::Deser(e.to_string()))?;

            info!("[GET_STATE_DELTA] Loaded state: {:?}", p);
            p
        };

        let ping_summary = if summary.is_empty() {
            info!("[GET_STATE_DELTA] Empty summary, using default Ping");
            Ping::default()
        } else {
            let ps = serde_json::from_slice::<Ping>(summary.as_ref())
                .map_err(|e| ContractError::Deser(e.to_string()))?;

            info!("[GET_STATE_DELTA] Loaded summary: {:?}", ps);
            ps
        };

        ping.merge(ping_summary, opts.ttl);

        info!("[GET_STATE_DELTA] Merged result: {:?}", ping);
